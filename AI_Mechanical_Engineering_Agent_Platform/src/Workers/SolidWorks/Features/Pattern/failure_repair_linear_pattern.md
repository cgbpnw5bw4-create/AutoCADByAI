...
定位并修复 `linear_pattern` Feature 的失败，避免把未验证能力误判为缺陷，也避免以放宽校验的方式"修好"问题。

## 适用范围

真实执行或 dry-run 返回 线性阵列相关 `failure_stage` 时使用。
...
| failure_stage | 含义 | 处理 |
|---|---|---|
| `feature_api_unverified` | 线性阵列 无真实执行证据 | **预期行为，不是缺陷。** 需按 `api_evidence.md` 的六项条件采集证据后才能执行 |
...
- 修复后 `dotnet test` 全绿。
- self-check 中 `linear_pattern_handler_registered` 与 `unverified_feature_blocks_execution` 均为 true。
